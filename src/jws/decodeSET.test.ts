...
describe("getJWSParts", () => {
  ...
  it("returns empty strings for missing parts", () => {
    expect(getJWSParts(header)).toEqual({ header, payload: "", signature: "" });
  });
});
...
describe("decodeSET", () => {
  ...
  it("throws FailedToDecodeSET when a part is not valid JSON", () => {
    const badPayload = Buffer.from("not-json").toString("base64url");
    expect(() => decodeSET(`${header}.${badPayload}.${signature}`)).toThrow("FailedToDecodeSET");
  });
});
